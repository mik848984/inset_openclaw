...
import {DuckbarImageResult, getVQD, NewsResult, SafeSearchType, SearchTimeType, VideoResult} from "duck-duck-scrape";
import {
    CallbackDuckbarPayload,
    CallbackNextSearch,
    CallbackSearchResult,
    DuckbarNewsResult, DuckbarRelatedSearch, DuckbarVideoResult
} from "duck-duck-scrape/lib/types";
import {COMMON_HEADERS, ensureJSON, queryString} from "duck-duck-scrape/lib/util";
...
export async function search(query: string, options?: SearchOptions, needleOptions?: NeedleOptions): Promise<SearchResults> {
    ...
    // const result = await fetch(`https://duckduckgo.com/?q=${query}&t=h_&ia=web`, {});
    //
    ...
    if (imagesMatch) {
        ...
        results.images = imagesResult.results.map((i: any) => {
    ...
    if (newsMatch) {
        ...
        results.news = newsResult.results.map((article: any) => ({
            date: article.date,
